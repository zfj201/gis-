...
export function buildSemanticSystemPrompt(layers: LayerDescriptor[]): string {
  ...
  const section3 = [
    ...
    `search/buffer_search 的 limit 必须 <= ${maxLimit}；若用户未明确“前N/N个”，默认设为 ${maxLimit}。`,
    `nearest 的 limit 必须 <= ${nearestMax}；若用户未明确“前N/N个”，默认设为 ${nearestDefault}。`,
  ...
  const section4 = [
    ...
    "admin 字段（如 county/区县）的 value 只能保留行政值本体，不得携带图层实体尾巴。",
    "in/not in 的 value 只允许纯集合元素，禁止带“中/里/内/之中/列表/清单/名录”等尾巴词。",
